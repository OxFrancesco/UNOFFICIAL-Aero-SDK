...
Run `bun run --cwd packages/sugar test` from the repository root to include the scene collision tests. The package pins Three.js 0.180.0 and the tests load the checked-in GLB by default. `AERO_SCENE_MODEL` can select another model for verification.
