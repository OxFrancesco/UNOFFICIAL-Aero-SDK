...
test('full TUI navigation prioritizes liquidity and carries custom weights into a reviewed rebalance', async () => {
  ...
  try {
    ...
    expect(home.indexOf('Pools')).toBeLessThan(home.indexOf('Create pool / add liquidity'))
    expect(home.indexOf('Create pool / add liquidity')).toBeLessThan(home.indexOf('Stocks'))
    expect(home).toContain('Quit')
    await capture('home')
    await key('RETURN')
    ...
    await key('n', true)
    expect(ui.captureCharFrame()).toContain('Create pool / add liquidity')
